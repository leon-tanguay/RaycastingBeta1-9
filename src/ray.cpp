...
namespace rc
{
    RayHandler::RayHandler(const Map& wallMap, sf::RenderWindow& window, const float GUI_SCALE) :
        window(window),
        wallMap(wallMap),
        ...
        RAY_IMPACT_COLOR_2D(Color::White),
        hitCircle{ 2.f }
    {
    ...
    void RayHandler::drawFloor(const float verticalPixelOffset)
    {
    ...
    Vector2f RayHandler::toTileCoords(const Vector2f& screenPos) const
    {
        return screenPos / TILE_SIZE;
    }

    float RayHandler::getVerticalPixelOffset(const float playerAngleVertical) const
    {
        return (SCREEN_SIZE.y / 90.f) * playerAngleVertical;
    }

    std::tuple<Vector2f, float> RayHandler::setUpRayAngle(const Vector2f& playerDir, const Vector2f& plane, const int x) const
    {
    ...
    Vector2f RayHandler::getStepSize(const Vector2f& rayDir) 
    {
        return Vector2f{ sqrt(1 + (rayDir.y / rayDir.x) * (rayDir.y / rayDir.x)),
                sqrt(1 + (rayDir.x / rayDir.y) * (rayDir.x / rayDir.y)) };
    }

    Vector2i RayHandler::getStep(const Vector2f& rayDir) 
   {
        Vector2i step;
        if (rayDir.x < 0)
        {
    ...
    Vector2f RayHandler::getInitialRayLength1d(const Vector2f& rayDir, const Vector2f& rayOrigin, const Vector2i& mapPos, const Vector2f& stepSize) const
    {
    ...
    std::tuple<Color, Vector2f, bool> RayHandler::castRayUntilHit(const Vector2f& rayLength1dInit, const Vector2i& mapPosInit, const Vector2f& stepSize, const Vector2i& step) const
    {
        bool side;
        Color rayColor;
    ...
    float RayHandler::getRayDistPerpendicular(const Vector2f& rayLength1d, const Vector2f& stepSize, const bool side) const
    {
        return (!side) ? rayLength1d.x - stepSize.x : rayLength1d.y - stepSize.y;
    }

    void RayHandler::shadeColor(Color& color, const float rayDistPerpendicular, const bool side) const
    {
    ...
    void RayHandler::drawVerticalBar3d(Color& color, const float rayDistPerpendicular, const float angleOffset, const float verticalPixelOffset, int x)
    {
    ...
    Vector2f RayHandler::getHitPos(const Vector2f& rayOrigin, const Vector2f& rayDir, const float rayDistPerpendicular, const float angleOffset) const
    {
    ...
    sf::VertexArray RayHandler::getRayLine2d(const Vector2f& rayOrigin, const Vector2f& hitPos) const
    {
    ...
    void RayHandler::draw2dRayLines()
    {
    ...
    void RayHandler::render(const Vector2f screenPos, const Vector2f playerDir, const float playerAngleVertical)
    {
